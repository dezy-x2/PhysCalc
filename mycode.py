...
def fillInChart(Vx=f_f, dx=f_f, t=f_f, vfy=f_f, dy=f_f):
    ...
    if not dy == f_f and not dx == f_f:
        t = ((-viy) - (((viy ** 2) - (4 * (0.5 * ay) * (-dy))) ** 0.5)) / \
            (2 * (0.5 * ay))
        vx = (dx / t)
        vfy = viy + (ay * t)
        answer = print("Your time is " + str(t) + " your final and initial velocity for the x axis are " +
                       str(vx) + " and the final velocity for your y axis is " + str(vfy * -1))
    elif not Vx == f_f and not dx == f_f:
        t = (dx / ((Vx + Vx) / 2))
        dy = (viy * t) + 0.5 * ay * (t ** 2)
        vfy = ((viy ** 2) + 2 * ay * dy) ** 0.5
        answer = print("Your time is " + str(t) + " Your distance on the y plane in " +
                       str(dy) + " Your velocity final on the y plane is " + str(vfy * -1))
    elif not t == f_f and not dx == f_f:
        Vx = (dx / t)
        dy = (viy * t) + 0.5 * ay * (t ** 2)
        vfy = ((viy ** 2) + 2 * ay * dy) ** 0.5
        answer = print("Your velocity along the x axis is " + str(Vx) + " You distance along the y axis is " +
                       str(dy) + " your velocity final along the y planes is " + str(vfy * -1))
    elif not t == f_f and not Vx == f_f:
        dx = ((Vx + Vx) / 2) * t
        dy = (viy * t) + 0.5 * ay * (t ** 2)
        vfy = ((viy ** 2) + 2 * ay * dy) ** 0.5
        answer = print("The distance traveled on the x plane is " + str(dx) + " the distance traveled along the y plane is " +
                       str(dy) + " the final velocity along the y plane is " + str(vfy * -1))
    elif not Vx == f_f and not vfy == f_f:
        dy = ((vfy ** 2) - (viy ** 2)) / (2 * ay)
        t = dy / ((viy + vfy) / 2)
        dx = ((Vx + Vx) / 2) * t
        answer = print("The distance traveled along the y axis is " + str(dy) + " the time taken is " +
                       str((t**2)**0.5) + " The distance traveled along the x axis is " + str(dx))
    elif not vfy == f_f and not dx == f_f:
        dy = ((vfy ** 2) - (viy ** 2)) / (2 * ay)
        t = dy / ((viy + vfy) / 2)
        Vx = (dx/t)
        answer = print("The distance traveled along the y axis is " + str(dy) +
                       " the time is " + str((t**2)**0.5) + " the velocity along the x axis is " + str(Vx))
    elif not dy == f_f and not Vx == f_f:
        vfy = ((viy ** 2) + 2 * ay * dy) ** 0.5
        t = dy / ((viy + vfy) / 2)
        dx = ((Vx + Vx) / 2) * t
        answer = print("The final velocity along the y axis is " + str(vfy * -1) + " the time is " +
                       str((t**2)**0.5) + " the distance along the x axis is " + str((dx ** 2)**0.5))
    return answer
...
def testConservationOfMomentumE():
    v2f = conservationOfMomentumE(
        m1=5, m2=7, v1i=4.5, v2i=-2, v1f=1.5, np=True)
...
def whatEquation():
    ...
    if choice == "getForce":
        ...
        vf = num(input("What is the final velocity? "))
        vi = num(
            input("What is the initial velocity (if you don't have one just put 0)? "))
        ...
        vi = num(
            input("What is your initial velocity (if you don't have this just put 0)? "))
        ...
        conservationOfMomentumE(m1=m1, m2=m2, v1i=v1i,
                                v2i=v2i, v1f=v1f, v2f=v2f)
        ...
        vf = num(input("What is the final velocity? "))
        vi = num(
            input("What is the initial velocity (if you don't have this just put 0)? "))
